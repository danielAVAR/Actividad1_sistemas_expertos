...
base_de_conocimiento = [
    ...
    # ── DESAFÍO NIVEL 1: Tres reglas nuevas ──────────────────
    {
        "id": "R08",
        "descripcion": "Falla de batería en laptop",
        "condiciones": ["es_laptop", "no_carga_bateria", "funciona_con_cable"],
        "conclusion": "Reemplazar la batería del portátil",
        "confianza": 0.89
    },
    {
        "id": "R09",
        "descripcion": "Problema de conectividad WiFi",
        "condiciones": ["enciende", "sin_internet", "otras_redes_si_funcionan"],
        "conclusion": "Reinstalar drivers de red y restablecer configuración TCP/IP",
        "confianza": 0.78
    },
    {
        "id": "R10",
        "descripcion": "Fallo del sistema operativo",
        "condiciones": ["enciende", "error_al_iniciar_so", "no_entra_a_windows"],
        "conclusion": "Reparar el arranque con la herramienta de recuperación de Windows o reinstalar el SO",
        "confianza": 0.83
    },
]
...
def resolver_conflictos(conflict_set):
    ...
    Retorna la lista completa ordenada de mayor a menor confianza
    (DESAFÍO NIVEL 2: múltiples diagnósticos).
    """
    if not conflict_set:
        return []
    return sorted(
...
def inferir(base_conocimiento, hechos):
    ...
    DESAFÍO NIVEL 2: muestra TODOS los diagnósticos posibles
    ordenados por confianza en lugar de solo el mejor.
    """
    print()
    print('━' * 55)
    ...
    # DESAFÍO NIVEL 2: ranking completo ordenado por confianza
    ranking = resolver_conflictos(conflict_set)
    regla_principal = ranking[0]

    print('  DIAGNÓSTICO PRINCIPAL')
    print('  ───────────────────────────────────────────────────')
    print(f'  Regla aplicada: {regla_principal["id"]} — {regla_principal["descripcion"]}')
    print(f'  Recomendación:  {regla_principal["conclusion"]}')
    print(f'  Confianza:      {regla_principal["confianza"] * 100:.0f}%')
    print()

    # Mostrar diagnósticos alternativos si los hay
    if len(ranking) > 1:
        print('  DIAGNÓSTICOS ALTERNATIVOS (ordenados por confianza)')
        print('  ───────────────────────────────────────────────────')
        for i, regla in enumerate(ranking[1:], start=2):
    ...
    print('  ───────────────────────────────────────────────────')
    print(' Síntomas que activaron el diagnóstico principal:')
    print(f'    {regla_principal["condiciones"]}')
    if len(ranking) > 1:
        descartadas = [r['id'] for r in ranking[1:]]
        print(f'  Diagnósticos de menor confianza descartados: {descartadas}')
    print('━' * 55)
...
PREGUNTAS = {
    ...
    # Preguntas para las nuevas reglas (Desafío Nivel 1)
    "es_laptop":                "¿El equipo es un portátil/laptop?",
    "no_carga_bateria":         "¿La batería no carga aunque esté conectada?",
    "funciona_con_cable":       "¿El equipo funciona normalmente conectado a la corriente?",
    "sin_internet":             "¿No hay conexión a internet en este equipo?",
    "otras_redes_si_funcionan": "¿Otros dispositivos en la misma red sí tienen internet?",
    "error_al_iniciar_so":      "¿Aparece un error o mensaje de fallo al iniciar Windows?",
    "no_entra_a_windows":       "¿El equipo no llega al escritorio de Windows?",
}
